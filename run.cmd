...
for %%f in (application.properties banner.txt logback.xml) do echo %%f: && copy "src\main\resources\%%f" target\
...
echo copying certificates to target:
for %%f in (*.p12) do echo %%f: && copy "src\main\resources\%%f" target\
echo copying config file(s) to target:
rem taking config from parent dir to avoid overwriting possibly adapted config at each startup
for %%f in (*.kmc) do echo %%f: && copy "%%f" target\
...
echo using config-file "%KISMON_HOME%\%configFile%":
